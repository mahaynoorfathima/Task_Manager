...
<center>
  ...
  <input style="border-color: red;border-radius: 10px;" type="text" name="name" placeholder="entert yor name" pattern="\w+[a-z]"
        title="User_id should only contain numbers "class="btn" >
   <input type="submit" name="submit" style="background-color: green;" value="DETAILS" >
<br><br><br><br>
</form>
</center>
</body>
...
{
   $name=$_POST['name'];
$sql = "SELECT * FROM messa WHERE   too='$name'";
...

{
  ...
  echo "<table border=4 align=center border-color=green>
    <tr><th>FROM</th>
    <th>EMAIL</th>
    <th>TO</th>
<th>MESSAGE</th>
<th>REPLY TO MANEGER</th>
    </tr>";
  while($row = mysqli_fetch_array($result))
  {
    echo "<tr><td>".$row['manager']."</td><td>".$row['email']."</td><td>".$row['too']."</td><td>".$row['message']."</td><td><a href=client_reply.php><button style=background-color:blue;color:white;width:100%;border-radius10px;>reply</button></td></a></tr>";
  }
  echo "</table>";
...
<html>
 ...
 <footer class="footer-expand-sm fixed" style="margin-top: 13%;">
    <center>
